...
void coveculjak::glava(int pecurka_pokrenut, int pecurka_parametar) {
	...
	if (pecurka_pokrenut)
		glColor3f(rand() / (float) RAND_MAX, 1.0 - rand() / (float) RAND_MAX,
...
void coveculjak::torzo(int pecurka_pokrenut, int pecurka_parametar) {
	...
	if (pecurka_pokrenut)
		glColor3f(1.0 - rand() / (float) RAND_MAX,
				  1.0 - rand() / (float) RAND_MAX, rand() / (float) RAND_MAX);
...
auto coveculjak::uhvatio_krticu(const std::vector<sargarepa> &sargarepice,
	...
	if (sargarepice.size() > 0) {
		for (unsigned i = 0; i < sargarepice.size(); i++) {
			...
			if (stranica1 >= k.first && stranica2 <= k.first &&
				stranica3 >= k.second && stranica4 <= k.second &&
				sargarepice[i].trenutno_zakljucana())
				return true;
		}
	}
	...
	if (rotkvice.size() > 0) {
		for (unsigned i = 0; i < rotkvice.size(); i++) {
			...
			if (stranica1 >= k.first && stranica2 <= k.first &&
				stranica3 >= k.second && stranica4 <= k.second &&
				rotkvice[i].trenutno_zakljucana())
				return true;
		}
	}
	...
	if (stranica1 >= k.first && stranica2 <= k.first && stranica3 >= k.second &&
		stranica4 <= k.second && magicna_pecurka.trenutno_zakljucana())
		return true;
...
void coveculjak::iscrtaj_na_poziciji(int pecurka_pokrenut,
...
auto coveculjak::kretanje(int pecurka_pokrenut,
						  int pecurka_parametar,
						  bool igra_zavrsena,
						  const std::vector<sargarepa> &sargarepice,
						  const std::vector<rotkvica> &rotkvice,
	...
	if (smer && m_poz + m_timer_vrednost <= m_granica_plus &&
		m_poz + m_timer_vrednost >= m_granica_minus) {
		if (hodaj(smer, pecurka_pokrenut, pecurka_parametar, igra_zavrsena,
				  sargarepice, rotkvice, magicna_pecurka))
			return false;

	}
	// Provera kada se krece paralelno sa x osom
	else if (!smer && m_pox + m_timer_vrednost <= m_granica_plus &&
			 m_pox + m_timer_vrednost >= m_granica_minus) {
		if (hodaj(smer, pecurka_pokrenut, pecurka_parametar, igra_zavrsena,
				  sargarepice, rotkvice, magicna_pecurka))
			return false;
	...
	// Ako je dosao do granice
		...
		if (smer) {
			if (m_ind1)
				m_poz += m_timer_vrednost + 0.02;
			else
				m_poz += m_timer_vrednost - 0.02;
		} else {
			if (m_ind1)
				m_pox += m_timer_vrednost + 0.02;
			else
				m_pox += m_timer_vrednost - 0.02;
		}
		...
		// Ukoliko odabere levo/desno kretace se levo ili desno u zavisnosti od
		// toga do koje je ivice stigao, ne moze preci preko granice kvadrata
		// koji mu je zadat
		srand(time(nullptr));
		smer = (2 * (rand() / (float) RAND_MAX) >= 1.0 ? true : false);

		// F-ovi sluze da bi poredjenje uopste bilo moguce, inace ne radi kako
		// treba
		if (m_pox == (m_granica_minus + 0.01f) &&
			m_poz == (m_granica_minus + 0.01f)) {
			if (smer)
				ugao = 0;
			else
				ugao = 90;
		...
		else if (m_pox == (m_granica_minus + 0.01f) &&
				 m_poz == (m_granica_plus - 0.01f)) {
			if (smer) {
				m_ind1 = 1;
				m_timer_vrednost = -0.02;
				ugao = 180;
		...
		else if (m_pox == (m_granica_plus - 0.01f) &&
				 m_poz == (m_granica_plus - 0.01f)) {
			if (smer)
				ugao = 180;
			else
