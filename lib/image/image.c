...
void image_read(Image *image, const char *filename) {
	...
	if (bih.bitcount == 24)
		image->pixels =
			(char *) malloc(3 * bih.width * bih.height * sizeof(char));
	else if (bih.bitcount == 32)
		image->pixels =
			(char *) malloc(4 * bih.width * bih.height * sizeof(char));
	else {
		fprintf(stderr,
				"image_read(): Podrzane su samo slike koje po pikselu cuvaju "
				"24 ili 32 bita podataka.\n");
		exit(1);
	}
	assert(image->pixels != NULL);
	...
	if (bih.bitcount == 24)
		...
		for (i = 0; i < bih.width * bih.height; i++) {
			/*
			 * Ovo mozda izgleda cudno, to sto se komponente boje citaju u
			 * suprotnom redosledu, tj. prvo plava, pa zelena, pa crvena, ali
			 * tako pise u specifikaciji bmp formata.
			 */
		 ...
		 * da oni (ta 4 bajta) predstavljaju R, G, B i A komponentu boje (1 bajt
		 * po komponenti).
		 */
		for (i = 0; i < bih.width * bih.height; i++) {
			fread(&b, sizeof(char), 1, file);
