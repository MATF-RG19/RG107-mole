...
// Indikator koji sluzi da onemoguci otkljucavanje krtice onda kada je igra
// gotova
	...
	for (int i = 0; i < 6; i++) {
		...
		for (int j = 0; j < 6; j++) {
			...
			if (2 * (rand() / (float) RAND_MAX) > 1.0) {
				// Da li ce povrce biti sargarepa ili rotkvica u slucaju da se
				// iscrtava
				if (2 * (rand() / (float) RAND_MAX) > 1.0) {
...
static auto provera_zakljucavanja(const mole &krtica,
								  const vector<sargarepa> &sargarepice,
								  const vector<rotkvica> &rotkvice,
								  const pecurka &magicna_pecurka) -> bool {
	bool a = false;

	if (sargarepice.size() > 1) {
		for (unsigned i = 0; i < sargarepice.size(); i++)
			if (sargarepice[i].poklapaju_se_koordinate() &&
				!sargarepice[i].pojedena())
				a = true;
	}

	if (rotkvice.size() > 1) {
		for (unsigned i = 0; i < rotkvice.size(); i++)
			if (rotkvice[i].poklapaju_se_koordinate() &&
				!rotkvice[i].pojedena())
				a = true;
	}

	if (magicna_pecurka.poklapaju_se_koordinate() &&
		!magicna_pecurka.pojedena())
		a = true;
...
// i koordinate koje predstavljaju pozitivnu i negativnu (tj.
// levu,desnu,godrnju,donju posto je kvadrat u pitanju ne trebaju nam sve
// cetiri) stranicu kvadrata po kom treba da se krecu
...
static void on_display() {
	...
	if (pecurka_pokrenut && pecurka_parametar > 10)
		glColor3f(1.0 - rand() / (float) RAND_MAX, rand() / (float) RAND_MAX,
				  1.0 - rand() / (float) RAND_MAX);
	glutSolidCube(1);
	glColor3f(0, 0, 0);
	glutWireCube(1);
	glPopMatrix();
	...
	if (!sargarepice.empty()) {
		...
		for (unsigned i = 0; i < n; ++i)
			iscrtaj_sargarepu(sargarepice[i], pecurka_pokrenut,
							  pecurka_parametar);
	}
	...
	// Kretacnje seljaka
	bool krece_se1 =
		cikica1.kretanje(pecurka_pokrenut, pecurka_parametar, igra_zavrsena,
						 sargarepice, rotkvice, magicna_pecurka);
	bool krece_se2 =
		cikica2.kretanje(pecurka_pokrenut, pecurka_parametar, igra_zavrsena,
						 sargarepice, rotkvice, magicna_pecurka);

	if (!krece_se1 || !krece_se2)
		izgubio_si(krtica, cikica1, cikica2, vreme_pokrenut, pecurka_pokrenut,
				   igra_zavrsena, names, window_height, window_width);
	...
	if (!igra_zavrsena) {
		...
		glColor3f(1, 1, 1);
		ispis_fiksiranog_teksta(to_string(vreme), 70, 560, 0, window_height,
								window_width);
	}
	...
	pobedio_si(krtica, cikica1, cikica2, vreme_pokrenut, pecurka_pokrenut,
			   igra_zavrsena, names, window_height, window_width, sargarepice,
			   rotkvice, vreme);
...
static void on_keyboard(unsigned char key, int x, int y) {
	...
	case 'i':
		if (krtica.provera_otkljucano() &&
			provera_zakljucavanja(krtica, sargarepice, rotkvice,
								  magicna_pecurka))
			krtica.zakljucaj();
	...
	case 'u':
		if (!igra_zavrsena) {
			...
			if (magicna_pecurka.uvuci() &&
				magicna_pecurka.preostalo_koraka() == 0) {
...
auto main(int argc, char **argv) -> int {
	...
	// Indikator koji sluzi da ne dozvoli da se krtica otkljuca nakon zavrsene
	// igre
	igra_zavrsena = false;
